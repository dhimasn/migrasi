...
class PermohonanKomitUloDb
{
    ...
    public function __construct()
    {
        ...
        $this->pdb = new PermohonanTelDb();
    }

    public function GetMekanismeUloById($id_permohonan_komit_layanan)
    {
        ...
        UNION ALL
        SELECT mup.baris, mup.value, mup.id_permohonan_komit_layanan, mu.value as value_header, mu.urutan FROM p_mekanisme_ulo_proses mup left join m_mekanisme_ulo mu on mup.id_mekanisme_ulo = mu.id where mup.id_permohonan_komit_layanan=%d
        ) temp order by temp.baris asc
    ...
    public function GetMekanismeUlo($id_permohonan)
    {
        ...
        try
        {
            ...
            foreach($result_layanan as $rl)
            {
                ...
                $q = sprintf("SELECT * from p_penomoran_tel_pakai ptp left join m_penomoran_tel_list ptl on ptp.id_penomoran_tel_list = ptl.id left join m_penomoran_tel pt on ptl.id_penomoran_tel = pt.id left join k_layanan kl on kl.id = pt.id_layanan where ptp.id_perusahaan = %d and pt.id_layanan = %d and ptp.id_permohonan = %d", $result_header->id_perusahaan, $rl->id_layanan, $id_permohonan);
            ...
            $q = sprintf("SELECT * from p_ulo_catatan where id_permohonan_komit=%d", $result_layanan[0]->id_permohonan_komit);
            $catatan = DB::select($q);
    ...
    public function EditMekanismeUloProses($input)
    {
        try
        {
            //delete sebelumnya 
            $q = sprintf("DELETE from p_mekanisme_ulo_proses where id_permohonan_komit_layanan=%d and id_mekanisme_ulo=%d", $input->mekanisme_ulo_proses[0]->id_permohonan_komit_layanan, 2);
            $a = DB::delete($q);
            $q = sprintf("DELETE from p_evaluasi_ulo_proses where id_permohonan_komit_layanan=%d", $input->mekanisme_ulo_proses[0]->id_permohonan_komit_layanan);
            $a = DB::delete($q);
            $q = sprintf("DELETE from p_ulo_file where id_permohonan_komit_layanan=%d", $input->mekanisme_ulo_proses[0]->id_permohonan_komit_layanan);
            $a = DB::delete($q);
            ...
            //update to p_ulo
            $q = sprintf("UPDATE p_ulo set id_ulo_status=%d where id_permohonan_komit=%d", TypeUloStatus::Disposisi, $input->id_permohonan_komit);
            $a = DB::insert($q);
